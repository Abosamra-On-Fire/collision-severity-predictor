...
def feature_interactions(
    ...
    try:
        ...
        new_df["casualties_per_vehicle"] = new_df["number_of_casualties"] / (
            new_df["number_of_vehicles"] + 1
        )
...
def feature_scaling_fit(df: pd.DataFrame, numerical_cols: list) -> ColumnTransformer:
...
def feature_scaling_transform(df: pd.DataFrame, preprocessor: ColumnTransformer) -> pd.DataFrame:
    new_df = df.copy()
    new_df = preprocessor.transform(new_df)
    new_df.columns = [col.split("__")[1] if "__" in col else col for col in new_df.columns]
    return new_df
...
def feature_encoding_fit(df: pd.DataFrame, categorical_cols: list) -> BinaryEncoder:
...
def feature_encoding_transform(df: pd.DataFrame, preprocessor: BinaryEncoder) -> pd.DataFrame:
    new_df = df.copy()
    new_df = preprocessor.transform(new_df)
    new_df.columns = [col.split("__")[1] if "__" in col else col for col in new_df.columns]
    return new_df
...
def variance_thresholding_fit(
    df: pd.DataFrame, min_variance: float, numerical_cols: list
) -> VarianceThreshold:
...
def variance_thresholding_transform(
    df: pd.DataFrame, selector: VarianceThreshold, numerical_cols: list
) -> tuple:
    new_df = df.copy()
...
def correlation_based_selection(
    X: pd.DataFrame, y: pd.Series, threshold: float, numerical_cols: list
) -> tuple:
...
def summarize(df_before: pd.DataFrame, df_after: pd.DataFrame, save_path: str):
    summary = []
    ...
    summary.append(["mean_of_means", num_before.mean().mean(), num_after.mean().mean()])

    summary.append(["mean_of_stds", num_before.std().mean(), num_after.std().mean()])
...
def main_features():
    ...
    X_train_with_interactions, numerical_cols_interaction, categorical_cols_interaction = (
        feature_interactions(X_train, numerical_cols, categorical_cols)
    )
    ...
    scaling_preprocessor = feature_scaling_fit(
        X_train_with_interactions, numerical_cols_interaction
    )
    ...
    selector = variance_thresholding_fit(
        X_train_scaled_encoded, cfg.VARIANCE_THRESHOLD, numerical_cols_interaction
    )

    (
        X_train_after_variance_thresholding,
        X_train_selected_numerical,
        numerical_cols_thresholding,
    ) = variance_thresholding_transform(
        X_train_scaled_encoded, selector, numerical_cols_interaction
    )
    X_val_after_variance_thresholding, X_val_selected_numerical, _ = (
        variance_thresholding_transform(X_val_scaled_encoded, selector, numerical_cols_interaction)
    )
    ...
    log_action(
        ...
        stage="feature_engineering",
        rule="",
        records_affected=len(X_train_after_variance_thresholding)
        + len(X_val_after_variance_thresholding),
        action="Columns Deletion",
        rationale="For each correlated pair of columns, we drop the least correlated with the target",
    )
    X_train_corr, numerical_cols_correlation, dropped_cols = correlation_based_selection(
        X_train_after_variance_thresholding,
        y_train,
